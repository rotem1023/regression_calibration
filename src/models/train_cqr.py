...
def pinball_loss(pred, target, gamma=0.9):
    ...
    loss = (target - pred) * gamma if pred < target else (pred - target) * (1.0 - gamma)
    ...
    try:
        for e in range(epochs):
            ...
            for batch_idx, (data, targets) in enumerate(tqdm(train_loader)):
                ...
                loss = pinball_loss(t, targets, gamma=gamma).to(device)
                loss.backward()
                epoch_train_loss.append(loss.item())
                optimizer_net.step()

                targets_train.append(targets.detach().cpu())
                t_train.append(mu.detach().cpu())
            ...
            with torch.no_grad():
                for batch_idx, (data, targets) in enumerate(tqdm(valid_loader)):
                    data, targets = data.to(device), targets.to(device)
                    t = model(data, dropout=True)
                    loss_valid = pinball_loss(t, targets, gamma=gamma).to(device)
            ...
            print(f"Epoch {e}:")
            print(f"train: loss: {epoch_train_loss:.5f}, pinball: {loss:.5f}")
            print(f"valid: loss: {epoch_valid_loss:.5f}, pinball: {loss_valid:.5f}")
            ...
            if is_best:
                ...
                filename = f"C:\lior\studies\master\projects\calibration\regression calibration\regression_calibration\models\cqr\{base_model}_{gamma}_{dataset}_cqr_best.pth.tar"
        ...
        save_current_snapshot(base_model, gamma, dataset, e, model, optimizer_net, train_losses, valid_losses)

    except KeyboardInterrupt:
        save_current_snapshot(base_model, gamma, dataset, e-1, model, optimizer_net, train_losses, valid_losses)
