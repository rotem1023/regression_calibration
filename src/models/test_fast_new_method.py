...
def calc_optimal_q_minus_plus(target_calib, mu_calib, sd_calib, alpha):
    small_examples= mu_calib[mu_calib >= target_calib]
    large_examples= mu_calib[mu_calib <= target_calib]
    sd_calib_small = sd_calib[mu_calib >= target_calib]
    sd_calib_large = sd_calib[mu_calib <= target_calib]
    target_calib_small = target_calib[mu_calib >= target_calib]
    target_calib_large = target_calib[mu_calib <= target_calib]
    
    s_t_small = (small_examples-target_calib_small) / sd_calib_small
    s_t_large = torch.abs(large_examples-target_calib_large) / sd_calib_large

    s_t_small_sorted, _ = torch.sort(s_t_small, dim=0)
    s_t_large_sorted, _ = torch.sort(s_t_large, dim=0)
    
    q_index_small = math.ceil((len(s_t_small_sorted)) * (1-alpha))
    q_index_large = math.ceil((len(s_t_large_sorted)) * (1 - alpha))
    
    q_small = s_t_small_sorted[q_index_small].item()
    q_large = s_t_large_sorted[q_index_large].item()
...
def calc_stats_new_method(q_minus, q_plus, target, mu, sd):
    lower = mu - q_minus * sd
    upper = mu + q_plus * sd
    length = torch.mean(abs(upper - lower))
    coverage = avg_cov(lower, upper, target)
...
def eval_test_set(save_params=False, load_params=False, mix_indices=True, calc_mean=False, save_test=False, load_test=False):
    ...
    if not use_saved_arrays:
        ...
        logvars_calib_original = torch.tensor(np.load(f'{results_dir}/lumbar_dataset_model_{base_model}_level{level}_logvars_calib_original.npy'))
        y_p_calib_original = torch.tensor(np.load(f'{results_dir}/lumbar_dataset_model_{base_model}_level{level}_y_p_calib_original.npy'))
        vars_calib_original = torch.exp(logvars_calib_original)
        targets_calib_original = torch.tensor(np.load(f'{results_dir}/lumbar_dataset_model_{base_model}_level{level}_targets_calib_original.npy'))

        y_p_test_original = torch.tensor(np.load(f'{results_dir}/lumbar_dataset_model_{base_model}_level{level}_y_p_test_original.npy'))
        logvars_test_original = torch.tensor(np.load(f'{results_dir}/lumbar_dataset_model_{base_model}_level{level}_logvars_test_original.npy'))
        vars_test_original = torch.exp(logvars_test_original)
        targets_test_original = torch.tensor(np.load(f'{results_dir}/lumbar_dataset_model_{base_model}_level{level}_targets_test_original.npy'))
    # save test arrays
    results_dir = "/home/dsi/rotemnizhar/dev/regression_calibration/src/models/results/predictions/"
    ...
    q_minus_all = []
    q_plus_all = []
    avg_len_valid_all_new_method = []
    avg_cov_valid_all_new_method = []
    avg_len_valid_all_new_method = []
    avg_cov_valid_all_new_method = []
    ...
    for j in range(iters):
        ...
        q_minus, q_plus = calc_optimal_q_minus_plus(target_calib, mu_calib, sd_calib, alpha)
        
        valid_length_new_method, valid_coverage_new_method =  calc_stats_new_method(q_minus, q_plus, target_calib, mu_calib, sd_calib)
        test_length_new_method, test_coverage_new_method =  calc_stats_new_method(q_minus, q_plus, target_test, mu_test, sd_test)
        ...
        print(f'valid_length_new_method: {valid_length_new_method}, valid_coverage_new_method: {valid_coverage_new_method}')
        print(f'test_length_new_method: {test_length_new_method}, test_coverage_new_method: {test_coverage_new_method}')
        ...
        q_minus_all.append(get_float(q_minus))
        q_plus_all.append(get_float(q_plus))
        
        avg_len_valid_all_new_method.append(get_float(valid_length_new_method))
        avg_cov_valid_all_new_method.append(get_float(valid_coverage_new_method))
        avg_len_valid_all_new_method.append(get_float(test_length_new_method))
        avg_cov_valid_all_new_method.append(get_float(test_coverage_new_method))
    ...
    output_dir= '/home/dsi/rotemnizhar/dev/regression_calibration/src/results'
    ...
    with open(f'{output_dir}/{output_file}', "w") as f:
        ...
        # print and save new method metrics
        
        print(f"q minus new method mean: {statistics.mean(q_minus_all)}, q new method std: {statistics.stdev(q_minus_all)}")
        f.write(f"q minus new method mean: {statistics.mean(q_minus_all)}, q new method std: {statistics.stdev(q_minus_all)}\n")
        
        print(f"q plus new method mean: {statistics.mean(q_plus_all)}, q new method std: {statistics.stdev(q_plus_all)}")
        f.write(f"q plus new method mean: {statistics.mean(q_plus_all)}, q new method std: {statistics.stdev(q_plus_all)}\n")
        
        print(f"avg len new method mean: {statistics.mean(avg_len_valid_all_new_method)}, avg len new method std: {statistics.stdev(avg_len_valid_all_new_method)}")
        f.write(f"avg len new method mean: {statistics.mean(avg_len_valid_all_new_method)}, avg len new method std: {statistics.stdev(avg_len_valid_all_new_method)}\n")
        
        print(f"avg cov new method mean: {statistics.mean(avg_cov_valid_all_new_method)}, avg cov new method std: {statistics.stdev(avg_cov_valid_all_new_method)}")
        f.write(f"avg cov new method mean: {statistics.mean(avg_cov_valid_all_new_method)}, avg cov new method std: {statistics.stdev(avg_cov_valid_all_new_method)}\n")
        ...
        print(f"avg_len validation mean new method: {statistics.mean(avg_len_valid_all_new_method)}, avg_len validation std new method: {statistics.stdev(avg_len_valid_all_new_method)}")
        f.write(f"avg_len validation mean new method: {statistics.mean(avg_len_valid_all_new_method)}, avg_len validation std new method: {statistics.stdev(avg_len_valid_all_new_method)}\n")
        
        print(f"avg_cov validation mean new method: {statistics.mean(avg_cov_valid_all_new_method)}, avg_cov validation std new method: {statistics.stdev(avg_cov_valid_all_new_method)}")
        f.write(f"avg_cov validation mean new method: {statistics.mean(avg_cov_valid_all_new_method)}, avg_cov validation std new method: {statistics.stdev(avg_cov_valid_all_new_method)}\n")
