...
def get_model_lumbar(model_name, level, base_model, device, lambda_param =5, after = False, one_out = False):
...
def get_model_boneage(model_name, base_model, device, lambda_param =5, after = False, one_out = False):
    models_dir = '/home/dsi/rotemnizhar/dev/regression_calibration/src/models/snapshots'
    if base_model != None:  # distance model
        models_dir = f"{models_dir}_new"
        if one_out: # model plots one dim output
        ...
        checkpoint = torch.load(f'{models_dir}/{model_name}_gaussian_boneage_best.pth.tar', map_location=device)
    model.load_state_dict(checkpoint['state_dict'])
    model.eval()
    return model