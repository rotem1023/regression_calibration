...
class EndoVisDataset(Dataset):
    ...
    def __init__(self, data_dir, mode='train', scale=1.0, augment=False, preload=False):
        ...
        #   self._path_tree = sorted([o for o in os.listdir(data_dir) if os.path.isdir(os.path.join(data_dir, o)) and (o == 'Dataset2' or o == 'Dataset3')])
            self._path_tree = sorted([o for o in os.listdir(data_dir) if os.path.isdir(os.path.join(data_dir, o)) and (o == 'Dataset1' or o == 'Dataset2')])
        else:
            # self._path_tree = sorted([o for o in os.listdir(data_dir) if os.path.isdir(os.path.join(data_dir, o)) and o == 'Dataset4'])
            self._path_tree = sorted([o for o in os.listdir(data_dir) if os.path.isdir(os.path.join(data_dir, o)) and (o == 'Dataset3' or o == 'Dataset4')])
        ...
        for path in self._path_tree:
            ...
            total_dataset_len = len([row for _, row in df.iterrows() if row[0] > -1 and row[1] > -1])
            count_indices = -1
            for index, row in df.iterrows():
                if row[0] > -1 and row[1] > -1:
                    count_indices += 1
                    if mode == 'val' and count_indices < total_dataset_len // 2 or mode == 'test' and count_indices >= total_dataset_len // 2:
